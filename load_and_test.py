...
from lib.utils_svfr.svfr_utils import print_success, print_info, print_warning, print_error, pose_error, load_model, \
    store_obj, matcher_fast_ALIKE, model2channels, iteratePnP, seed_env, init_device, parse_args, create_dataloader, \
    create_tracker, redirect2log
from lib.utils_svfr.visualizer_utils import map_to_rgb, visualize_camera_poses
...
def test(model, tracker, cfg, visualization=False):
    ...
    # if cfg.data.net_model == 'superpoint':
    #     matcher_function = matcher_fast_SP
    # elif "aliked" in cfg.data.net_model:
    #     matcher_function = matcher_fast
    # else:
    #     matcher_function = matcher_fast_ALIKE
    matcher_function = matcher_fast_ALIKE
    ...
    with open(ground_truth_test_path, 'r') as file:
        ...
        for line in tqdm(lines):
            ...
            if visualization:
                ...
                for kp1, kp2, score in zip(estimated_kpts, target_kpts, scores):
                    ...
                    color = map_to_rgb((1.0 - score) / 0.1)
        ...
        print(f"Voxel-grid size: {cfg.coarse_model_and_render.num_voxels} voxels")
        print('-----------------------------------------')
...
if __name__ == '__main__':
    ...
    # print options
    print(f"Reprojection error: {cfg.data.reprojection_error[cfg.data.net_model]}")
    print(f"Feature matching threshold: {cfg.data.match_threshold[cfg.data.net_model]}")
    print(f"Voxel-grid size: {cfg.coarse_model_and_render.num_voxels} voxels")
    print(f"Patch size half: {cfg.data.patch_size_half}")
    ...
    # test(model, tracker, cfg, visualization=False)
