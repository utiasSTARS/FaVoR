...
__global__ void infer_t_minmax_cuda_kernel(
        ...
        const double near, const double far, const int64_t n_rays,
        scalar_t* __restrict__ t_min,
        scalar_t* __restrict__ t_max) {
  const int64_t i_ray = blockIdx.x * blockDim.x + threadIdx.x;
  if(i_ray<n_rays) {
    const int64_t offset = i_ray * 3;
    double vx = ((rays_d[offset  ]==0) ? 1e-6 : rays_d[offset  ]);
    double vy = ((rays_d[offset+1]==0) ? 1e-6 : rays_d[offset+1]);
    double vz = ((rays_d[offset+2]==0) ? 1e-6 : rays_d[offset+2]);
    double ax = (xyz_max[0] - rays_o[offset  ]) / vx;
    double ay = (xyz_max[1] - rays_o[offset+1]) / vy;
    double az = (xyz_max[2] - rays_o[offset+2]) / vz;
    double bx = (xyz_min[0] - rays_o[offset  ]) / vx;
    double by = (xyz_min[1] - rays_o[offset+1]) / vy;
    double bz = (xyz_min[2] - rays_o[offset+2]) / vz;
...
torch::Tensor infer_n_samples_cuda(torch::Tensor rays_d, torch::Tensor t_min, torch::Tensor t_max, const double stepdist) {
  const int64_t n_rays = t_min.size(0);
...
__global__ void sample_pts_on_rays_cuda_kernel(
        ...
        int64_t* __restrict__ ray_id,
        int64_t* __restrict__ step_id,
        const double stepdist, const int64_t total_len,
        scalar_t* __restrict__ rays_pts,
        bool* __restrict__ mask_outbbox) {
  const int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if(idx<total_len) {
    const int64_t i_ray = ray_id[idx];
    const int64_t i_step = step_id[idx];

    const int64_t offset_p = idx * 3;
    const int64_t offset_r = i_ray * 3;
    const double dist = stepdist * i_step;
    const double px = rays_start[offset_r  ] + rays_dir[offset_r  ] * dist;
    const double py = rays_start[offset_r+1] + rays_dir[offset_r+1] * dist;
    const double pz = rays_start[offset_r+2] + rays_dir[offset_r+2] * dist;
...
std::vector<torch::Tensor> sample_pts_on_rays_cuda(
        ...
        const double near, const double far, const double stepdist) {
  const int64_t threads = 256;
  const int64_t n_rays = rays_o.size(0);
  ...
  const int64_t total_len = N_steps.sum().item<int64_t>();
...
__global__ void sample_ndc_pts_on_rays_cuda_kernel(
        ...
        const int64_t N_samples, const int64_t n_rays,
        scalar_t* __restrict__ rays_pts,
        bool* __restrict__ mask_outbbox) {
  const int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if(idx<N_samples*n_rays) {
    const int64_t i_ray = idx / N_samples;
    const int64_t i_step = idx % N_samples;

    const int64_t offset_p = idx * 3;
    const int64_t offset_r = i_ray * 3;
    const double dist = ((double)i_step) / (N_samples-1);
    const double px = rays_o[offset_r  ] + rays_d[offset_r  ] * dist;
    const double py = rays_o[offset_r+1] + rays_d[offset_r+1] * dist;
    const double pz = rays_o[offset_r+2] + rays_d[offset_r+2] * dist;
    rays_pts[offset_p  ] = px;
    rays_pts[offset_p+1] = py;
    rays_pts[offset_p+2] = pz;
    mask_outbbox[idx] = (xyz_min[0]>px) | (xyz_min[1]>py) | (xyz_min[2]>pz) | \
...
__global__ void sample_bg_pts_on_rays_cuda_kernel(
   ...
   */
    const double t_inner = t_max[i_ray];
    const double ori_t_outer = t_inner - 1. + 1. / (1. - ((double)i_step) / N_samples);
    const double ori_ray_pts_x =  rays_o[offset_r  ] + rays_d[offset_r  ] * ori_t_outer;
    const double ori_ray_pts_y =  rays_o[offset_r+1] + rays_d[offset_r+1] * ori_t_outer;
    const double ori_ray_pts_z =  rays_o[offset_r+2] + rays_d[offset_r+2] * ori_t_outer;
    const double t_outer = norm3(ori_ray_pts_x, ori_ray_pts_y, ori_ray_pts_z);
    const double ori_ray_pts_m = max(abs(ori_ray_pts_x), max(abs(ori_ray_pts_y), abs(ori_ray_pts_z)));
    const double R_outer = t_outer / ori_ray_pts_m;
    const double o2i_p = R_outer*R_outer / (t_outer*t_outer) * (1.-bg_preserve) + R_outer / t_outer * bg_preserve;
    const double px = ori_ray_pts_x * o2i_p;
    const double py = ori_ray_pts_y * o2i_p;
    const double pz = ori_ray_pts_z * o2i_p;
    rays_pts[offset_p  ] = px;
    rays_pts[offset_p+1] = py;
    rays_pts[offset_p+2] = pz;
  }
...
torch::Tensor sample_bg_pts_on_rays_cuda(
        ...
        const double bg_preserve, const int64_t N_samples) {
  const int64_t threads = 256;
  const int64_t n_rays = rays_o.size(0);
...
bool check_xyz(int64_t i, int64_t j, int64_t k, int64_t sz_i, int64_t sz_j, int64_t sz_k) {
...
__global__ void maskcache_lookup_cuda_kernel(
    ...
    const int64_t sz_i, const int64_t sz_j, const int64_t sz_k, const int64_t n_pts) {

  const int64_t i_pt = blockIdx.x * blockDim.x + threadIdx.x;
  if(i_pt<n_pts) {
    const int64_t offset = i_pt * 3;
    const int64_t i = round(xyz[offset  ] * xyz2ijk_scale[0] + xyz2ijk_shift[0]);
    const int64_t j = round(xyz[offset+1] * xyz2ijk_scale[1] + xyz2ijk_shift[1]);
    const int64_t k = round(xyz[offset+2] * xyz2ijk_scale[2] + xyz2ijk_shift[2]);
...
torch::Tensor maskcache_lookup_cuda(
  ...
  const int64_t sz_i = world.size(0);
  const int64_t sz_j = world.size(1);
  const int64_t sz_k = world.size(2);
  const int64_t n_pts = xyz.size(0);
  ...
  const int64_t threads = 256;
  const int64_t blocks = (n_pts + threads - 1) / threads;
...
__global__ void raw2alpha_cuda_kernel(
    ...
    const double shift, const double interval, const int64_t n_pts,
...
__global__ void raw2alpha_nonuni_cuda_kernel(
    scalar_t* __restrict__ density,
    const double shift, scalar_t* __restrict__ interval, const int64_t n_pts,
    scalar_t* __restrict__ exp_d,
    scalar_t* __restrict__ alpha) {
...
std::vector<torch::Tensor> raw2alpha_cuda(torch::Tensor density, const double shift, const double interval) {

  const int64_t n_pts = density.size(0);
...
std::vector<torch::Tensor> raw2alpha_nonuni_cuda(torch::Tensor density, const double shift, torch::Tensor interval) {
...
__global__ void raw2alpha_backward_cuda_kernel(
    ...
    const double interval, const int64_t n_pts,
...
__global__ void raw2alpha_nonuni_backward_cuda_kernel(
    scalar_t* __restrict__ exp_d,
    scalar_t* __restrict__ grad_back,
    scalar_t* __restrict__ interval, const int64_t n_pts,
    scalar_t* __restrict__ grad) {
...
torch::Tensor raw2alpha_backward_cuda(torch::Tensor exp_d, torch::Tensor grad_back, const double interval) {

  const int64_t n_pts = exp_d.size(0);
...
__global__ void alpha2weight_cuda_kernel(
  ...
  const int64_t i_ray = blockIdx.x * blockDim.x + threadIdx.x;
  if(i_ray<n_rays) {
    const int64_t i_s = i_start[i_ray];
    const int64_t i_e_max = i_end[i_ray];
...
std::vector<torch::Tensor> alpha2weight_cuda(torch::Tensor alpha, torch::Tensor ray_id, const int64_t n_rays) {

  const int64_t n_pts = alpha.size(0);
  const int64_t threads = 256;
...
__global__ void alpha2weight_backward_cuda_kernel(
  ...
  if(i_ray<n_rays) {
    const int64_t i_s = i_start[i_ray];
    const int64_t i_e = i_end[i_ray];

    double back_cum = grad_last[i_ray] * alphainv_last[i_ray];
    for(int64_t i=i_e-1; i>=i_s; --i) {
...
torch::Tensor alpha2weight_backward_cuda(
        ...
        torch::Tensor i_start, torch::Tensor i_end, const int64_t n_rays,
