...
def log_extractor(dir, scene, measure_significant, net):
    ...
    for model in sorted(os.listdir(dir)):
        ...
        try:
            ...
            print_results(scene, model, init_dist_errors, init_angle_errors, estimated_dist_errors, estimated_angle_errors, measure_significant)
...
def print_results(scene, model, init_dist_errors, init_angle_errors, estimated_dist_errors, estimated_angle_errors, measure_significant):
    ...
    print(f"{scene_name} & 1st & {model_name} & {init_dist_median:.2f}/{init_angle_median:.2f} & {dist_medians[0]:.2f}/{angle_medians[0]:.2f}\\")
...
def print_results_per_network(results_voc, count, net):
    for key in net:
        ...
        print(f"Cambridge & 1st & {key} & {init_dist_error:.2f}/{init_angle_error:.2f} & {dist_error_1:.2f} / {angle_error_1:.2f}\\")
...
if __name__ == "__main__":
    ...
    for scene, percentage in sig_data:
        print(f"{scene}\t 3rd iter, <{measure_significant[dataset_type][0]:.0f} cm && <{measure_significant[dataset_type][1]:.0f} deg: {percentage:.2f}\%\\\\")
